...
public double getSalePrice(String makeAndModel, int yearManufactured, double milesDriven, int airBagsCount, boolean hasAbs, boolean hasEbd, boolean hasRearViewCamera, boolean hasSunRoof, 
...
if (ageOfCar <=10){
salePrice=salePrice-(0.05*ageOfCar*salePrice);
}
else {
...
if (hasRearViewCamera&&(hasSunRoof||hasAutoAC)){
	...
	double expectedMilesDriven=ageOfCar*1000.0;
	double additionalMiles=milesDriven-expectedMilesDriven;
	
	if (additionalMiles > 1000 &&additionalMiles <=1000){
		salePrice-=500;
	}
	...
	System.out.println("salePrice after acounting for milesDriven:" +salePrice);
	return salePrice;
...
public static void main (String args []) {
	CarEstimator carPriceEstimator = new CarEstimator();
	double salesPrice= carPriceEstimator.getSalePrice("ford ecosport", 2018, 20000.0, 2, true, false, true, false, false, true);
